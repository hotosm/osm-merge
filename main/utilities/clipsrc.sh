...
if test ! -e  ${country}-osm.geojson; then
    ...
    ogr2ogr -progress -f "GeoJSON"  ${country}-osm.geojson -sql "SELECT osm_id,tags,geom FROM ways_poly WHERE tags->>'building' IS NOT NULL" PG:"${dbhost}dbname=${country}"
fi

# MS Building footprins are in a geojson file
if test ! -e  ${country}-ms.geojson; then
    echo "Collecting MS buildings within ${country}..."
    ogr2ogr -progress  -sql "SELECT ogc_fid AS id,wkb_geometry AS geom FROM multipolygons" -f "GeoJSON" ${country}-ms.geojson PG:"${dbhost}dbname=${country}_foot"
fi
...
for project in ${projects}; do
    ...
    ogr2ogr -progress -f GeoJSON -sql "SELECT ogc_fid AS id,wkb_geometry AS geom FROM multipolygons" -clipsrc ${id}-project.geojson ${id}-ms.geojson PG:"${dbhost}dbname=${country}_foot"
done
