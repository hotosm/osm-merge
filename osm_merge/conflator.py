...
class Conflator(object):
    ...
    def checkTags(self,
        ...
        for key in match:
            ...
            if key == "name":
                if "name" in extfeat["properties"]:
                    if extfeat["properties"]["name"] == "Whiskey Park Road":
                        breakpoint()
                    elif "name" in osm["properties"]:
                        print(f"RATIOS: {ratio}: {osm["properties"]["name"]} == {extfeat["properties"]["name"]}")
        # print(props)
        return hits, props
